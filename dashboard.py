...
filtered_revenue = filtered_order_items.merge(filtered_orders, on='order_id') \
                                      .groupby(filtered_orders['order_date'].dt.strftime('%Y-%m')) \
                                      .agg({'quantity': 'sum', 'unit_price': 'sum'}) \
                                      .assign(total_amount=lambda x: x['quantity'] * x['unit_price']) \
                                      .reset_index()[['order_date', 'total_amount']]
